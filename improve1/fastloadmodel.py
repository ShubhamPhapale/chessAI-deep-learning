...
class MemoryEfficientTrainer:
    def __init__(self, model: nn.Module, criterion: nn.Module, 
        ...
        self.scaler = torch.amp.GradScaler()  # Updated this line
    ...
    def train_epoch(self, dataloader: DataLoader, accumulation_steps: int = 4) -> float:
        self.model.train()
        running_loss = 0.0
        ...
        for step, (board_states, move_labels) in enumerate(dataloader):
            ...
            with torch.amp.autocast(device_type='cuda'):  # Updated this line
                loss = self.train_step(board_states, move_labels)
            ...
            # Clear memory
            del board_states, move_labels, loss
            if step % 100 == 0:
                torch.cuda.empty_cache()
...
def create_dataloader(pgn_file: str, batch_size: int = 32, num_workers: int = 4, 
                     cache_size: int = 1000) -> DataLoader:
...
def train_model(pgn_file: str, model: nn.Module, epochs: int = 200, 
                batch_size: int = 32, num_workers: int = 4, cache_size: int = 1000):
    ...
    dataloader = create_dataloader(pgn_file, batch_size, num_workers, cache_size)
...
if __name__ == "__main__":
    ...
    config = {
        ...
        'num_workers': 4,
        'cache_size': 1000,
        'mode': 'train'  # or 'play' for game mode
    }
    ...
    try:
        ...
        if config['mode'] == 'train':
            ...
            if not os.path.exists(pgn_file):
                raise FileNotFoundError(f"PGN file not found at {pgn_file}")
            ...
            logging.info("Starting game mode")
            
            # Check if model file exists
            if not os.path.exists('chess_model_final.pth'):
                raise FileNotFoundError("Model file not found. Please train the model first.")
            
            play_game()
            
        else:
            raise ValueError(f"Invalid mode: {config['mode']}. Must be 'train' or 'play'")

    except KeyboardInterrupt:
        logging.info("Process interrupted by user")
        # Save checkpoint if in training mode
        if config['mode'] == 'train':
